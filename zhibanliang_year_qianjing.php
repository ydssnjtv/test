...
echo "<p style='color:black;font-size:32px;text-align:center;'>成功构造Mopaas Mysql数据库连接！<br/></p>";
...
echo "<p style='color:red;font-size:32px;text-align:center;'>你选择的时间是：".$year.'年<br/></p>';
panduan($time);
//具体查询和结果显示
...
while($i<=8){
  ...
  $count=mysql_fetch_row($aa);
   ...
   if($i==8){echo "</tr>";}//再次提醒！！！如果写$i=7就不对，比较关系中只有==，不能=；
   $i++;
}
...
while($i<=8){
  $ry=$renyuan[$i];
  $sql_ss = "select count(*) from `banbiaoqianjing` where `日期` BETWEEN '$time' AND '$timeend1' and `审晚` like '%$ry%'";//select语句里变量要用引号括起来！！！！这句执行的结果是某人本月所有早班的列表；
  $aa=mysql_query($sql_ss)or die("对不起，读入数据时出错了！". mysql_error());
  $count=mysql_fetch_row($aa);
   echo "<td><p style='color:black;font-size:35px;text-align:center'>".$count[0]."</p></td>";
   $summary[$ry]=$summary[$ry]+$count[0];
...
while($i<=8){
  $ry=$renyuan[$i];
  $sql_ss = "select count(*) from `banbiaoqianjing` where `日期` BETWEEN '$time' AND '$timeend1' and `收带` like '%$ry%'";//select语句里变量要用引号括起来！！！！这句执行的结果是某人本月所有收带班的列表；
  $aa=mysql_query($sql_ss)or die("对不起，读入数据时出错了！". mysql_error());
  $count=mysql_fetch_row($aa);
   echo "<td><p style='color:black;font-size:35px;text-align:center'>".$count[0]."</p></td>";
   $summary[$ry]=$summary[$ry]+$count[0];
