<table border="1" align="center">
 <tr>
  ...
  <th><span style='color:black;font-size:35px'>收带</span></th>
 </tr>
...
while($result=mysql_fetch_row($a2))//通过循环读取数据内容，mysql_fetch_array() 是 mysql_fetch_row() 的扩展版本。除了将数据以数字索引方式储存在数组中之外，还可以将数据作为关联索引储存，用字段名作为键名。也就是如果下标是字符串，会多返回一份下标是字符串的结果。
{
           ...
           if  ($zhouji=="日" or  $zhouji=="六") 
           {
			  echo "<tr bgcolor='black'>";
			   echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[0].'</p></td>';
			   echo "<td><p style='color:white;font-size:35px;text-align:center'>".$zhouji.'</p></td>';
			   echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[1].'</p></td>';
		       echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[2].'</p></td>';
		       echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[3].'</p></td>';
		       echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[4].'</p></td>';
		       echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[5].'</p></td>';
			   echo "<td><p style='color:white;font-size:35px;text-align:center'>".$result[6].'</p></td>';
            } else 
           {
                echo "<tr>";
				  ...
				  echo "<td><p style='color:black;font-size:35px;text-align:center'>".$result[6].'</p></td>';
            }
